...
#### 4.1.1 配置约束与默认值
1. 背景图文件约束
- 支持格式：`image/png`、`image/jpeg`、`image/webp`、`image/avif`
- 大小上限：`10MB`（超出后阻止保存并提示）
- 像素总数上限：`40MP`（超出后阻止保存并提示）
...
### 5.2 异常与反馈
1. 文件类型不支持：拦截并提示 `仅支持 PNG/JPG/WebP/AVIF`。
2. 文件超限（> 10MB）：拦截并提示 `图片大小不能超过 10MB`。
3. 图片像素超限（> 40MP）：拦截并提示 `图片尺寸不能超过 40 MP`。
4. 图片读取失败：提示 `图片加载失败，请重试`，保留上一次有效配置。
5. blur 输入越界：自动 clamp 到 `0-20` 并在 UI 显示矫正后的值。
...
7. 背景图上传时仅接受 PNG/JPG/WebP/AVIF，且文件大小超过 `10MB` 或像素总数超过 `40MP` 时可被拦截并给出提示。
